"""
Run many population-weighted random event reallocations and save combined output.
"""
...
# Output
OUT_DIR = Path("output")
OUT_PATH = OUT_DIR / "reallocated_random_pop_weighted_100_runs.pkl"
...
def _get_cell_ids_and_weights(
    ...
    if pop_col not in grid.columns:
        raise KeyError(f"grid must contain column '{pop_col}'")

    base = grid[[cell_id_col, pop_col]].copy()
    base = base.dropna(subset=[cell_id_col])

    # If duplicate cells exist, sum population to get a single weight per cell.
    grouped = base.groupby(cell_id_col, sort=False, as_index=False)[pop_col].sum()
    try:
        grouped = grouped.sort_values(cell_id_col, kind="mergesort")
    except Exception:
        pass

    cell_ids = grouped[cell_id_col].to_numpy()
    weights = grouped[pop_col].to_numpy(dtype=float)
    weights = np.clip(weights, 0.0, None)
    return cell_ids, weights


def _allocate_random_weighted(
    ...
    *,
    ...
    total_weight = float(np.sum(cell_weights)) if n_cells else 0.0

    def sample_cells(count: int) -> np.ndarray:
        if n_cells <= 0 or count <= 0:
            return np.array([], dtype=object)
        if total_weight <= 0:
            assigned_idx = rng.integers(0, n_cells, size=count, endpoint=False)
        else:
            probs = cell_weights / total_weight
            assigned_idx = rng.choice(n_cells, size=count, replace=True, p=probs)
        return cell_ids[assigned_idx].astype(object)
    ...
    def run_block(block_events: pd.DataFrame, category: Optional[str]) -> pd.DataFrame:
        ...
        assigned = sample_cells(E) if E > 0 else np.array([None] * E, dtype=object)
...
def main() -> None:
    ...
    cell_ids, cell_weights = _get_cell_ids_and_weights(grid, CELL_ID_COL, POP_COL)
    ...
    for run_id in range(N_RUNS):
        ...
        allocations = _allocate_random_weighted(
            events,
            cell_ids=cell_ids,
